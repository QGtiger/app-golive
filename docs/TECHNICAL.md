...
| 本地持久化 | userData 中的 JSON（凭据明文存放） | 用户已确认 |
...
- Renderer：表单、拖入、状态展示；不接触 Node.js 与文件系统（凭据由主进程读写后用于设置页回显）。
...
已核实的接口没有鉴权契约，不提供网关 Token / 登录流程。

### 4.3 本地保存

- 存放在 `app.getPath('userData')/golive.json`。
- 全部配置（含 OSS 凭据）明文 JSON 存放——纯本机工具，凭据不出本机；旧版 safeStorage 加密字段读取时自动解密迁移为明文。
- 凭据在设置页明文回显、可直接编辑；清空后保存即删除。
...
- 成功：`{ "success": true, "data": <应用记录> }`
- 业务错误：**HTTP 状态码同为 200**，body 为 `{ "success": false, "code": 404, "message": "未找到 app: <id>" }`

客户端以 `success`/`code` 判定错误（不依赖 HTTP 状态码），成功时解包 `data`。

**发布流程不做预查**：`app/deploy` 本身就是“无则创建、有则追加版本”（服务端 `deployApp` 保证），客户端直接发布即可。`app/get` 仅用于一个场景——deploy 网络失败后的结果确认（§9）：核对 `currentVersion` 与 `ossIndexUrl` 是否与本次发布一致。
...
3. 生成独立版本号（`Date.now()`，校验安全整数）与对象前缀。
4. 有脚本则执行，等待正常退出；退出码非零立即停止。
5. 扫描最终上传内容，验证入口与资源引用（HTML 须完整 URL）并生成文件清单。
6. 文件按原始路径直传，不修改、不复制、不改写任何内容。
7. 上传全部成功后调用 `app/deploy` 登记版本；请求始终携带建议 `domain`（仅新建生效，服务端对已有应用忽略）。
8. 校验响应中的 id、当前版本、入口 URL，生成实际访问地址。
9. 保存项目配置并展示结果；本地保存失败不能误报远程发布失败。

建议版本号使用 `Date.now()` 并校验安全整数。本机重复概率低，跨端冲突由服务端的重复版本拒绝兜底。
...
客户端**只校验不改写**：改写 HTML/CSS 无法覆盖 JS 内部的动态引用，做到一半反而制造"看起来能跑"的假象。产物必须自洽，资源正确性由构建期保证——发布脚本环境注入 `GOLIVE_ASSET_BASE`（`{publicRoot}/{appId}/{version}/`），业务构建时显式使用它：

```sh
pnpm exec vite build --base "$GOLIVE_ASSET_BASE"
```

发布前校验规则（`collectFiles` 内执行，命中即发布失败，错误信息含具体引用与修复指引）：

- **HTML**：`src` / `srcset` / `poster` / `<link href>` 的值必须是完整 URL（`http(s)://` 或协议相对 `//`）或 `data:` 等协议——页面由网关在自有域名回源，相对与根路径引用都会请求到网关域名下而 404；
- **CSS**：相对地址相对 CSS 文件自身在 OSS 的位置解析、天然自洽，放行；仅根路径 `url(/...)`、`@import "/..."` 拒绝；
- **JS**：不扫描、不改写——字符串无法区分资源路径与 API 路径，误报代价过高。

限制：SPA 的前端路由子路径（如 `/detail/1`）刷新是否可用取决于网关回源策略；`<a>` 站内导航同理。不承诺"任意 Web 项目零配置"，需要框架特殊部署配置时按错误提示调整构建。
...
- 文件按原始路径直传，Content-Type 由 SDK 依扩展名推断。
- 上传使用有限并发；SDK 请求设置超时。
